...
1. Read [references/visual-direction-contract.md](references/visual-direction-contract.md) and [references/visual-readiness-contract.md](references/visual-readiness-contract.md).
...
4. Set the brand-authorship mode. Preserve every supplied name, Logo, color, and approved VI rule as an invariant. For `logo_and_color` or `logo_only`, author the missing typography, neutral/supporting roles, form language, imagery, crop, density, layout posture, and interaction style from current project evidence. For `no_brand_assets`, create a named project brand hypothesis when the project or evaluation allows brand creation; otherwise preserve the customer name and author the missing visual system. Create two compact internal direction candidates that differ in at least three visual-signature dimensions. Keep them as contract-level decisions or small composition sketches, not two complete Figma systems. Select one using product, audience, journey, content, asset, and implementation evidence; record why the rejected candidate fits less well. Ask UI only when both remain materially valid and the choice changes the customer-facing outcome.
...
12. Validate the representative set at screenshot scale without relying on the written contract. It must communicate the product type, selected brand direction, media strategy, content differences, and responsive behavior. Revise before full-page production when it remains generic, under-designed, disconnected from source/theme evidence, uses repeated visuals for different meanings, or requires UI to re-decide the first screen or core media composition.
