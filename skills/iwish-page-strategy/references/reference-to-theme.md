...
Read [layout-topology-contract.md](layout-topology-contract.md). Create composition-group IDs `G01`, `G02`, and so on for connected source experiences before mapping individual sections. Section boundaries must follow rendered composition and interaction, not a semantic content checklist.
...
- `composed_native`: two or more evidenced native sections/blocks reproduce one source section without changing its visible boundary, topology, order, content, interaction, or responsive transformation.
...
- Reproduce Desktop and Mobile topology separately, including major-region proportions, visible-item counts, overflow, controls, and responsive transformation.
