...
- For theme customization, resolve `target_theme_input` before the Theme Capability Map. Use the exact provided theme when present. When no definite theme is provided, compare current paid themes and automatically select the closest fit from product/category, page responsibilities, functional requirements, selected source topology, responsive behavior, the Visual Direction Brief, and customization burden. Do not pause for approval or default to an easily inspected free theme. For fully custom work, mark theme selection and mapping not applicable.
