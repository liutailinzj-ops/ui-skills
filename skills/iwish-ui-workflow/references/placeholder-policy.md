...
- Do not render unsupported specification groups as repeated `—`, `-`, `N/A`, empty value cells, or visually complete tables with no useful values. Collapse optional rows, replace the section with truthful qualitative decision support, or keep the unfinished table in an internal replacement record.
