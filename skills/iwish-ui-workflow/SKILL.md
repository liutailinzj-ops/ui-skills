...
5. Load `$iwish-visual-direction`. Translate the approved strategy into a project-specific Visual Direction Contract, evidence-backed visual signature, project-authored brand direction, visual-asset plan, representative responsive section families, and only the breakpoint proof states needed to verify their behavior. Preserve supplied brand invariants; when VI is light or absent, author the missing typography, neutral palette, form language, imagery, crop, density, and interaction rules from the current product, audience, market, and journey instead of waiting for more customer material. Resolve two deliberately different internal direction candidates before selecting one; do not build two complete Figma directions or ask UI to choose unless the evidence cannot resolve a material fork. Do not create independent Desktop/Mobile art directions or continue from a generic wireframe, repeated geometric placeholders, or an undocumented house style.
...
8. Load `$iwish-figma-page-builder` to extend the representative visual language into one complete editable responsive page system with Desktop and Mobile preview states. Before full-page assembly, validate the canonical customer-visible content ledger and the selected representative set with final styles, content, and real or generated temporary visual assets. The set must prove the primary conversion/first-screen composition, one content-differentiation module, and one imagery-led module. Do not continue when semantically different products, configurations, features, or scenes are represented by the same generic image or abstract primitive. Build one Section family at a time and preserve shared content, component identity, brand, source, theme, and evidenced breakpoint behavior.
