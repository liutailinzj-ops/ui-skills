...
6. When `theme_state` is `to_be_selected`, research and select the best-supported theme candidate before the Theme Capability Map. When a theme is already specified, audit it. For `reference_to_theme`, fingerprint the specified structure source, produce its Source Page Specification and Layout Topology Contract with composition groups, then map each source section to separate field-level theme capability evidence. Do not map a theme demo back to itself.
...
- In `reference_to_theme`, preserve captured composition groups and normalized layout topology. Do not split a connected source experience into independent generic sections or count semantic topic coverage as layout fidelity.
...
- A `reference_to_theme` run has a verified Source Identity Fingerprint, Source Page Specification, Layout Topology Contract, composition groups, and cross-source Theme Assembly Plan; Build Truth URL identity, product/category identity, source section count, order, content-item coverage, composition-group coverage, Desktop/Mobile topology coverage, and mapping status pass at 100% unless a deviation has explicit approval provenance.
