...
已有项目进行局部调整时，UI 仍调用同一个 `$iwish-ui-workflow` 并直接说明要改的页面、模块或节点。Workflow 会读取现有 Manifest 和项目视觉方向，只重跑受影响的 Skill、响应式配对和局部 QA，不默认重做整页。
...
全部就绪案例产出独立视觉结果后，还需运行 `evals/rubrics/cross-case-bias-review.md`，排查相同配色、字体、首屏拓扑、页面节奏、组件语法或图片处理是否由 Skill 自身无依据地重复产生。
...
- UI 负责创意方向、品牌判断和最终微调。
- 技术部负责平台、主题和实现边界。
- Visual Direction Contract 是项目级视觉主规范；客户确认信息和已批准的局部覆盖优先，检索结果、竞品观察和模型推荐只能作为候选，不能静默覆盖既有项目规则。
- 只有 Logo/品牌色等轻品牌资料时，AI 先在内部形成两套至少三个视觉维度不同的方向候选，只选择一套进入 Figma；不会生成两套完整页面增加 UI 工作量。
- 客户预览的文字、图片和控件统一来自客户可见内容账本，审批状态、占位来源、替换负责人和实现备注只保留在内部数据中。
- 完整页面铺开前，必须先用最终样式和实际内容验证一个高风险响应式 Section，确认文字边界、导航、裁切、重复项和溢出行为。
