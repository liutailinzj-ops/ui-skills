...
- 案例 A1：指定结构 + 模板建站 + 严格复刻，验证无法精确映射时会诚实阻断。
- 案例 A2：同一指定结构 + 模板建站 + 主题适配，验证能够保留内容责任和购买链路并继续生成可编辑 Figma。
- 案例 B：无指定结构 + 模板建站，验证自主研究和主题选择。
- 案例 C：指定结构 + 模板二开，验证无法原生实现时能否诚实阻断。
- 案例 D–F：资料不足 DTC、纯定制 DTC 和 B2B，待补充真实项目夹具。

Fiido/Concept 只作为案例 A1/A2，不得将其行业、区块数量、视觉风格、主题名称或具体布局写进通用 Skill。一次修改只有在全部就绪案例重新运行后，才能说明回归套件通过；单个案例通过不能证明生产稳定。
...
- 日常指定结构的模板项目默认使用“主题适配”：完整捕获来源内容与顺序，保留关键功能、组合关系和阅读链路；主题原生导致的列数、交互或局部几何差异继续生成，并列入 UI 微调清单。
- 只有客户明确要求 1:1、完全一致或不可调整时才使用“严格复刻”；严格模式中的未解决差异会在 Figma 前阻断。
