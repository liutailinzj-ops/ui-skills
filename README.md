...
- 模板部分二开只输入“已提供确定模板”或“未提供确定模板”。已提供时严格使用该模板；未提供时由 Skill 按品类、页面职责、功能、竞品结构、响应式行为、视觉方向和二开成本自动匹配合适的当前付费模板，不询问谁选择或确认。纯定制标记为“不适用”。
- 不因 Dawn 等免费模板代码易获取而默认使用；只有明确提供该模板或项目明确允许免费模板时才可采用。
...
- 代表性 Section、整页构图板和组件库只用于生成前预检；完整生产必须交付同一响应式系统的完整 Desktop 与 Mobile 页面，二者都通过后才能报告完成。
